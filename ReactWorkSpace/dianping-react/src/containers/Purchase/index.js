...
class Purchase extends Component {
  render() {
    const { phone, product, showTip, quantity, totalPrice } = this.props;
    ...
    return (
      <div className="purchase">
        ...
        {showTip && <Tip message="购买成功!" onClose={this.handleCloseTip} />}
      </div>
    );
  }
  componentDidMount() {
  ...
  // 提交订单
  handleSubmit = () => {
    const { purchaseActions, match } = this.props;
    const productId = match.params.id;
    purchaseActions.submitOrder(productId);
  };
  // 设置购买数量
